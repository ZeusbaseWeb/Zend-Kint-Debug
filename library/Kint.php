...
/**
 * Kint Debugging library
 * @author Rokas Sleinius
...
class Kint
{
    ...
    /**
     * @var string base dir to Kint library
     */
    ...
    /**
     * Initialises Kint's configuration from it's config file
     * @static
     * @author Laurynas Karvelis
     *
     * @param array $overridenConfigs
     *
     * @return void
     */

    public static function init($overridenConfigs = array())
    {
        if(isset($overridenConfigs['enabled'])) {
            $overridenConfigs['enabled'] = (bool) $overridenConfigs['enabled'];
        }

        $config = self::_getDefaultConfig();
        $config = array_merge($config, $overridenConfigs);
        ...
        self::$_baseDir = dirname(__FILE__) . '/' . __CLASS__ . '/';
    }

    /**
     * Loads default configuration settings for Kint library
     * @static
     * @author Laurynas Karvelis
     * @param void
     * @return array
    ...
    public static function trace($trace = NULL)
    {
        ...
        include self::$_baseDir . 'resources/trace.phtml';
    }
    ...
    private static function _css()
    {
        ...
        $ret .= '<script>';
        $ret .= file_get_contents(self::$_baseDir . '/resources/' . (self::$devel ? 'src/' : '') . 'kint.js');
        $ret .= '</script>';

        $ret .= '<style>';
        $ret .= file_get_contents(self::$_baseDir . '/resources/' . (self::$devel ? 'src/' : '') . self::$skin);
        $ret .= '</style>';
